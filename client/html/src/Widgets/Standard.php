...
namespace Aimeos\Client\Html\Swordbros\Widgets;
	...
	implements \Aimeos\Client\Html\Common\Client\Factory\Iface
{
	...
	private $subPartPath = 'client/html/swordbros/widgets/standard/subparts';
	...
	public function getBody( string $uid = '' ) : string
	{
		...
		$confkey = 'client/html/swordbros/widgets';

		if( ( $html = $this->getCached( 'body', $uid, $prefixes, $confkey ) ) === null )
		{
			$view = $this->getView();
			$tplconf = 'client/html/widgets/'.$uid;
			$default = 'widgets/'.$uid.'/body-standard';
	...
	public function getSubClient( string $type, string $name = null ) : \Aimeos\Client\Html\Iface
	{
		...
		return $this->createSubClient( 'swordbros/widgets/' . $type, $name );
	}
	...
	public function process()
	{
		...
		try
		{
			...
			$catId = $context->getConfig()->get( 'client/html/swordbros/widgets/catid-default' );
			...
			$context->getSession()->set( 'aimeos/swordbros/widgets/params/last/' . $site, $params );
	...
	public function addData( \Aimeos\MW\View\Iface $view, array &$tags = [], string &$expire = null ) : \Aimeos\MW\View\Iface
	{
		...
		$types = $config->get( 'swordbros.widgets.types', ['sw_widgets'] );

		$domains = $config->get( 'swordbros.widgets.domains', ['text', 'media'] );
        ...
        $view->content =  '';
        if($Page){
            foreach($Page->toArray() as $widgets ){ 
                foreach($widgets->getListItems() as $page){ 
