...
<section class="checkout-standard-payment ">
	...
	<div class="note">
	<span class="checkout-section-title"><?= $enc->html( $this->translate( 'client', 'payment' ), $enc::TRUST ); ?></span> - <?= $enc->html( $this->translate( 'client', 'Please choose your payment method' ), $enc::TRUST ); ?></div>
				...
				<label class="description" for="c_paymentoption-<?= $enc->attr( $id ); ?>">
					...
					<?php foreach( $service->getRefItems( 'text', null, 'default' ) as $textItem ) : ?>
						...
						<?php if( ( $type = $textItem->getType() ) !== 'name' ) : ?>
							...
							<p class="<?= $enc->attr( $type ); ?> m-0 p-0 font-14 font-weight-500"><?= $enc->html( $textItem->getContent(), $enc::TRUST ); ?></p>
