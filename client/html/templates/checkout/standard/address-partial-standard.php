...
<?php /*?><div class="row form-item form-group address2 <?= $enc->attr( ( $this->value( 'error', 'order.base.address.address2' ) ? 'error ' : '' ) . join( ' ', $this->value( 'css', 'order.base.address.address2', [] ) ) ) ?>"
		...
		<div class="col-md-7">
			<select class="form-control" id="address-<?= $this->get( 'type', 'billing' ) ?>-countryid-<?= $this->get( 'id' ) ?>"
				...
				<?php if( count( $this->get( 'countries', [] ) ) > 1 ) : ?>
					<option value="RU" selected><?= $enc->html( $this->translate( 'country', 'RU') ); ?></option>
				<?php endif; ?>
	...
	<div class="row form-item form-group languageid <?= $enc->attr( ( $this->value( 'error', 'order.base.address.languageid' ) ? 'error ' : '' ) . join( ' ', $this->value( 'css', 'order.base.address.languageid', [] ) ) ) ?>"
		...
		<div class="col-md-7">
			<select class="form-control" id="address-<?= $this->get( 'type', 'billing' ) ?>-languageid-<?= $this->get( 'id' ) ?>"
				...
				<?php $lang =Route::current()->parameter('locale');?>  <?php \App::setLocale($lang)?>

				<?php if( count( $this->get( 'languages', [] ) ) > 1 ) : ?>
					<option value="<?=$lang?>" selected> <?= $enc->html( $this->translate( 'language', $lang ) ); ?></option>
				<?php endif; ?>
