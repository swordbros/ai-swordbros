...
<section class="aimeos basket-standard table table-responsive" data-jsonurl="<?= $enc->attr( $this->url( $optTarget, $optCntl, $optAction, [], [], $optConfig ) ); ?>">
    ...
    color: black;
    font-size: 18px;
"><?= $enc->html( $this->translate( 'client', 'My Basket' ), $enc::TRUST ); ?> / <?= $enc->html( $this->translate( 'client', 'Details' ), $enc::TRUST ); ?></h3>
		...
		<form method="POST" action="<?= $enc->attr( $this->url( $basketTarget, $basketController, $basketAction, [], [], $basketConfig ) ); ?>">
			...
			<div class="common-summary-detail  table table-responsive">
			...
			<div class="button-group">
				...
				<?php if( isset( $this->standardBackUrl ) ) : ?>
					...
					<button class="btn btn-default btn-lg btn-back" href="<?= $enc->attr( $this->standardBackUrl ); ?>">
				...
				<?php if( $this->get( 'standardCheckout', false ) === true ) : ?>
					...
					<a class="btn btn-primary btn-lg btn-action text-white"
