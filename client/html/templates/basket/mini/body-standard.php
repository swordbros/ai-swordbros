...
?>
<section class="aimeos basket-mini" data-jsonurl="<?= $enc->attr( $this->url( $jsonTarget, $jsonController, $jsonAction, ( $basketSite ? ['site' => $basketSite] : [] ), [], $jsonConfig ) ); ?>"  style="display: initial;">
		...
		<div class="basket-mini-product">
			...
			<table class="basket">
				...
				<tfoot class="basket-footer">
					<tr class="delivery">
							<?php $value= $enc->html( sprintf( $this->number( $priceItem->getValue() + $priceItem->getCosts(), $priceItem->getPrecision() ) ) );?>
					<?php if (intval($value) == 0 ) : ?>
						<td class="basket-empty" colspan="4">
							<a><?= $this->translate( 'client', 'Your basket is empty' );?> </a>
						</td>
					<?php else : ?>
