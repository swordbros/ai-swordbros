	...
	implements \Aimeos\Client\JsonApi\Iface
{
	...
	public function get( ServerRequestInterface $request, ResponseInterface $response ) : \Psr\Http\Message\ResponseInterface
	{
		...
		try
		{
            if($view->param( 'set' )){
                ...
                $default = 'review/unauthorized';
                if($this->getContext()->getUserId()){
                    $tplconf = 'client/jsonapi/review/standard/template';
                    $default = 'review/add';
                    $response = $this->saveItem( $view, $request, $response );
                }
            } else{
                $tplconf = 'client/jsonapi/review/standard/template';
	 ...
	 * @return \Psr\Http\Message\ResponseInterface Modified response object
	 */
	protected function saveItem( \Aimeos\MW\View\Iface $view, ServerRequestInterface $request, ResponseInterface $response ) : \Psr\Http\Message\ResponseInterface
	{
        $_token = $view->param( '_token' );
        $session = app('session');

        if ( !(isset($session) && $_token == $session->token())) {
            return $response;
        }
        ...
        $data['review.product_id'] = $view->param( 'review_product_id' );
        $data['review.user_id'] = $this->getContext()->getUserId();
        $data['review.review'] = html_entity_decode($view->param( 'review_review' )) ;
        $data['review.rating'] = $view->param( 'review_rating' );

        $data['review.label'] = 'Review Awaiting Approval';
        $data['review.status'] = '-1';
        $data['review.code'] = md5($data['review.review']);
        $cntl->uses( $ref )->saveItem(null, $data);
        return $response;
	}
	...
	protected function getItems( \Aimeos\MW\View\Iface $view, ServerRequestInterface $request, ResponseInterface $response ) : \Psr\Http\Message\ResponseInterface
	{
		...
		$items = \Aimeos\Controller\Frontend::create( $this->getContext(), 'review' )
